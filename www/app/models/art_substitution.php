...
class ArtSubstitution extends AppModel
{
	var $name = 'ArtSubstitution';
	...
	var $belongsTo = array(
		'MedicalInformation' => array(
			...
			'foreignKey' => FALSE,
			'conditions' => array('ArtSubstitution.pid = MedicalInformation.pid ')
		),
		'Regimen' => array(
			'className'  => 'Regimen',
			'foreignKey' => FALSE,
			'conditions' => array('ArtSubstitution.art_starting_regimen_id = Regimen.id')
			)
	);
	...
	var $validate = array(
		...
		'art_substitution_reason_id'=>array(
			'positive integer' => array(
				'rule'    => array('customValidationFunction', 'isPositiveInteger'),
				'message' => 'The Patient ID must be a positive integer'
			),
